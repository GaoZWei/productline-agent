...
- `blocking_stage=NONE`与根因互斥；其他阶段必须有根因，防止一边声称无阻塞、一边又给出故障
  原因。正式阻塞阶段枚举仍留给M2.6，M2.4没有提前实现业务判断规则。
...
回答要点：M2.4只定义数据契约，具体阶段集合与规则属于M2.6。当前先限制为大写稳定代码并校验
`NONE`根因互斥，避免跨任务实现；M2.6加入枚举时需同步Schema和规则测试。
...
- M2.5已实现LangGraph事实加载节点，但仍没有确定性规则、诊断API或前端诊断卡片。
- `Evidence`校验来源和形状，但尚未自动验证字段路径确实存在于本次Tool响应；该能力要在Workflow
  节点或评测中实现。
- `confidence`只有0～1范围校验，尚未实现置信度计算、校准或评测，不能声称结果可信度已量化。
- 正式`blocking_stage`枚举和ORDER-001～005分支规则属于M2.6，当前不能声称五个场景均已诊断。
...
按什么顺序取、失败后停在哪里”，为M2.6规则和后续Agent动态选路提供可重复基线。
...
- 当前只完成事实加载到`load_delivery`，M2.6阻塞阶段规则和M2.7诊断结果/文案尚未实现。
