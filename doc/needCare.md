...
Python Agent 不直接读取或修改 Java 业务数据库。页面传入的订单、任务和用户信息也只能作为上下文提示，服务端仍需重新校验。

### 需要掌握的设计取舍

- Java 保持业务规则和数据一致性，避免模型或 Python 编排层绕过业务约束。
- Python 更适合承载模型调用、Tool 协议和动态工作流，但不能成为业务事实来源。
- Web 提供的 `order_id` 不能直接信任，否则“这个订单”可能被错误上下文或越权请求解析到不应访问的数据。
...
回答要点：直接查询会绕开权限、状态语义、审计和兼容层；数据库结构变化还会直接破坏 Tool。Java API 应提供经过校验的稳定业务事实，Agent 只通过结构化 Tool 使用。

**为什么 Java 和 Python 要拆成两个服务？**

回答要点：Java 承载已有业务能力和强一致性约束，Python 承载模型生态与编排；拆分不是为了技术栈数量，而是为了明确“事实和写入由谁负责”。
...
- Java Enum、DTO 和数据库 `CHECK` 使用相同的大写状态字符串，避免 Java、Python和前端各自维护一套含义不同的状态。
- Entity 负责持久化关系，DTO 负责未来接口契约；后续 API 不应直接序列化 JPA
  懒加载实体。
- `Order` 作为聚合根维护任务和交付关系，关系方法阻止子对象跨订单或跨任务重挂。
- 模型只能归纳事实。例如模型可以根据 `OPEN + PENDING + BLOCKED` 判断质量复核阻塞，但不能自行生成不存在的复核结果。

### 可能的面试问题

**领域模型和 Agent Tool 有什么关系？**

回答要点：Tool 的输入输出 Schema 应来源于稳定业务契约。领域状态不统一时，模型会面对模糊或冲突字段，Prompt 再复杂也无法保证可靠判断。

**为什么不能直接把 JPA Entity 返回给 Tool？**

回答要点：Entity 含持久化和懒加载语义，容易暴露内部结构、产生循环引用或随数据库重构变化；接口 DTO 才是跨服务稳定边界。
...
核心关注。M0.3 的价值不是“写了初始化 SQL”，而是为后续 Tool 调用、诊断路径和Agent 回归评测建立可重复的业务事实基线。
...
回答要点：模型输出可能变化，但底层业务事实和预期结论必须确定；否则无法判断错误来自模型、Prompt、Tool、数据还是环境，也无法进行稳定回归。

**这五个订单和普通测试夹具有什么区别？**

回答要点：它们不仅验证数据库映射，还代表 Agent 的关键决策分支。后续可以基于它们评估 Tool 选择、事实一致性、根因判断和建议是否匹配业务状态。

**如何降低 Agent 补造业务事实的风险？**

回答要点：业务事实只来自 Java Tool；模型负责路由、归纳和解释；用固定场景验证输出必须能追溯到 Tool 结果，写操作还需要人工确认和 Java 再校验。

**数据库约束和状态一致性校验为什么都需要？**

回答要点：外键、枚举等单表结构由数据库保证；“存在 OPEN 问题但交付 READY”属于跨对象语义，需要业务层判定，并在未来写接口提交前执行。
...
## M2.4 结构化诊断契约与Tool证据边界

### 面试价值判断

核心关注。Agent系统不能只输出“因为质检有问题”这类不可验证文案。M2.4把诊断拆成阻塞阶段、
结构化根因、Tool字段证据、建议和置信度，并明确区分静态Workflow State与运行时Pydantic校验，
为后续规则、前端展示和评测建立可验证契约。
...
Java事实
→ 只读Tool Pydantic输出
→ OrderDiagnosisState中的业务状态通道
→ 确定性规则（M2.6已实现）
→ DiagnosisResult
   ├── RootCause
   ├── Evidence(source_type/tool_name/field_path/value)
   └── Suggestion
```

`Evidence`当前只允许七个已实现只读Tool作为来源，`value`必须是单个标量字段，不能用模型判断或
整段业务响应冒充证据。这样后续可以检查“结论是否真的由已调用Tool的具体字段支持”。

### 需要掌握的原理和设计取舍

- `OrderDiagnosisState`使用`TypedDict`，适合LangGraph式节点共享字典和mypy静态检查，但运行时
  不会自动拒绝错误字典；跨边界的业务事实和最终诊断仍由Pydantic负责运行时校验。
- Workflow内部Schema统一`extra=forbid/frozen/strict`，避免未知字段静默传播、结果对象被随意
  重写以及字符串数字被隐式转换。
- 根因和建议使用稳定机器代码加人类说明，程序可以基于代码评测或展示，文案可以独立演进。
- 证据要求`source_type=TOOL`、已知Tool名、可定位字段路径和标量值，体现“模型负责解释，Tool
  提供事实”的边界；未来增加RAG证据时应使用新的来源类型和引用结构，不应复用Tool字段语义。
- `blocking_stage=NONE`与根因互斥；其他阶段必须有根因，防止一边声称无阻塞、一边又给出故障
  原因。M2.4当时没有跨任务提前实现业务判断，正式枚举现已由M2.6补充并同步到结果契约。
- `StepError`只携带稳定错误码、安全文案、retryable和Trace ID，不接受原始响应或异常堆栈，
  避免敏感数据通过Workflow状态扩散。

### 可能的面试问题及回答要点

**TypedDict和Pydantic为什么同时使用？**

回答要点：TypedDict零运行时包装，适合节点共享状态和静态类型检查；Pydantic用于Tool输出、最终
诊断等可信边界的运行时校验。只用TypedDict挡不住运行时脏数据，只用BaseModel承载每次节点状态
更新又会增加转换和合并成本。

**如何减少Agent诊断幻觉？**

回答要点：不让自由文本成为唯一依据。业务事实先经过Java API和Tool Schema，诊断证据必须记录
已知Tool、字段路径和标量值；模型只能归纳解释，不能成为业务状态来源。后续评测还可按字段路径
核对结论。

**为什么不把完整Tool响应放进Evidence？**

回答要点：完整响应难以定位真正支持结论的字段，也会放大Token、隐私和存储风险。字段级标量证据
更容易审计、展示和自动评测；完整事实仍保留在当前Workflow状态中，不需要复制到每条证据。

**为什么M2.4没有直接定义阻塞阶段枚举？**

回答要点：M2.4只定义数据契约，具体阶段集合与规则属于M2.6，因此当时先限制为大写稳定代码并
校验`NONE`根因互斥。M2.6实现时再一次性加入枚举、规则决策通道和参数化测试，避免契约先于规则
失去真实语义。

### 简历表述建议

> 设计Agent结构化诊断契约，以TypedDict承载节点共享状态、Pydantic校验根因/建议/错误；将诊断
> 证据约束到已注册Tool、响应字段路径和标量值，明确模型解释与业务事实边界，为幻觉控制、前端
> 溯源和自动评测提供基础。

### 不能过度声称

- M2.6已实现确定性阶段规则，但仍没有最终诊断文案、诊断API或前端诊断卡片。
- `Evidence`校验来源和形状，但尚未自动验证字段路径确实存在于本次Tool响应；该能力要在Workflow
  节点或评测中实现。
- `confidence`只有0～1范围校验，尚未实现置信度计算、校准或评测，不能声称结果可信度已量化。
- 正式`blocking_stage`枚举和ORDER-001～005规则分支已由M2.6单元测试覆盖，但尚未完成五订单真实
  Java HTTP到诊断API的E2E，不能声称五个场景已端到端交付。
- 当前没有RAG引用，不能把Tool字段证据描述为规范文档引用。

---

## M2.5 固定Workflow：确定性取证、失败中断与Step短事务

### 面试价值判断

核心关注。M2.5把七个固定加载节点真正编译为LangGraph图，让业务事实只从已校验Tool进入共享
状态，并在首个标准错误处停止后续取证。同时把每次上下文/Tool动作接入Step生命周期，形成可定位、
不跨网络持锁的执行证据。这能直接回答Agent Workflow可靠性、状态合并和可观测性设计问题。
...
Run级ToolContext
→ load_context初始化状态
→ load_order/tasks/progress/quality/review/delivery
→ ToolResult.data按task_id合并
├── 成功：进入下一固定节点
└── 失败：ToolError → StepError → errors → END
```

模型没有参与这条路径，节点也不能直接访问Java数据库。这样在引入LLM前先证明“取了哪些事实、
按什么顺序取、失败后停在哪里”，并已成为M2.6规则和后续Agent动态选路的可重复基线。

### 需要掌握的原理和设计取舍

- 固定流程选择LangGraph `StateGraph`而不是自由Agent循环：当前问题的取证顺序已知，固定边更容易
  测试、复现和控制调用预算；动态决策留到M5，避免过早让模型决定关键事实链路。
- 节点只返回增量状态，由LangGraph合并；多任务结果使用`dict[task_id, result]`保留归属，并按
  `task_id`稳定排序，降低上游顺序变化造成的非确定性。
- 多任务节点先在局部字典收集全部结果，全部成功后才返回更新；失败时不把半份节点聚合写入状态，
  但此前已成功的Tool Step仍保留为执行证据。
- `ToolResult.error`转换为受控`StepError`并走条件边到`END`，而不是继续调用无意义的下游Tool；
  错误码、retryable和Trace ID仍可供上层Run/API决定最终响应。
- 每个Workflow实例绑定一个`ToolContext`且只运行一次，保证M1.7调用账本确实属于单次Run，避免
  独立上下文让重复调用检测失效。
- `DatabaseWorkflowStepRecorder`在Tool前后使用两个独立短事务：先提交RUNNING Step并释放父Run
  锁，再执行HTTP，最后提交成功/失败。它避免网络延迟扩大数据库锁范围。
- Step摘要只保存ID、数量、状态和错误码等白名单字段，不复制完整Java响应或Token；摘要脱敏仍是
  纵深防御，不应成为随意落库原始载荷的理由。

### 可能的面试问题及回答要点

**为什么这个阶段用固定Workflow，不直接用Agent自主调用Tool？**

回答要点：订单未交付诊断的基础取证顺序明确，先用固定图建立确定性基线，能验证Tool契约、错误
路径、调用预算和状态归属。后续动态Agent只能在已验证Tool和评测基线上做选路，不能用模型自由度
替代业务正确性。

**LangGraph节点为什么返回partial state，而不是整份state？**

回答要点：节点只拥有自己负责的通道，返回增量可以降低互相覆盖历史事实的风险，职责更清楚。
多任务数据用task_id字典合并，测试同时锁定节点顺序和聚合结果。

**一个Tool失败后为什么立即结束？**

回答要点：下游诊断依赖完整可信事实；继续调用会增加延迟和成本，还可能基于缺失信息产生误导。
标准错误被保留为StepError，已完成状态和Step证据不丢失，上层以后可选择重跑而不是伪造诊断。

**为什么Step记录不能和HTTP调用放在同一数据库事务？**

回答要点：M2.3开始Step时会锁父Run。若事务覆盖整个HTTP等待，慢请求会长时间阻塞Run终态更新。
因此开始和结束分别短事务提交，网络调用在事务外执行；代价是进程崩溃可能留下RUNNING Step，需
后续恢复任务处理。

### 简历表述建议

> 基于LangGraph实现确定性订单诊断取证Workflow，按订单与任务归属合并Java Tool强类型事实，
> 将标准错误转换为可路由StepError并在失败节点中断；通过短事务适配Run/Step生命周期，避免
> 外部HTTP等待期间持有数据库锁，为后续规则诊断和执行追踪建立可测试基线。

### 不能过度声称

- 当前已在`load_delivery`后执行M2.6阻塞阶段规则，但M2.7诊断结果/文案尚未实现。
- 没有诊断HTTP API、前端Agent侧栏或Run创建/终态编排，不能声称ORDER-003已可从前端完整诊断。
- Step短事务可能在进程崩溃后留下RUNNING记录，尚无超时回收、恢复、checkpoint或exactly-once。
- Workflow实例内序号分配只适用于当前线性单次执行，尚未支持并行分支、断点恢复或同Run追加执行。
- LangGraph当前没有checkpointer，Agent Step表是业务可观测记录，不等同于LangGraph状态持久化。
- 这条固定图没有LLM、RAG或动态Tool选择，不能包装为自主Agent能力。
...
核心关注。M2.6把六种业务结果和信息不足编码成可重复测试的纯规则，让模型只能解释结论，不能
决定或篡改阻塞阶段。它直接体现Agent工程里“业务裁决确定性、自然语言表达可生成”的分层设计，
也能说明如何避免缺失Tool事实时产生看似完整的幻觉答案。
...
```text
已通过Tool Schema校验的OrderDiagnosisState
→ 完整性与父子归属检查
├── 不完整：INSUFFICIENT_INFORMATION
└── 完整：按最早阶段优先判断
    PRODUCTION_BLOCKED → PRODUCTION → QUALITY_REVIEW → REVIEW → DELIVERY → NONE
→ RuleDecision(order_id, blocking_stage)
→ M2.7再生成根因、Evidence和Suggestion
```

五个固定订单的规则夹具分别锁定`PRODUCTION/PRODUCTION_BLOCKED/QUALITY_REVIEW/REVIEW/NONE`；
ORDER-003明确命中`QUALITY_REVIEW`。这个结果来自代码规则，不依赖模型采样或Prompt措辞。

### 需要掌握的原理和设计取舍

- `BlockingStage`使用七值`StrEnum`而不是自由字符串，让Workflow、Schema、测试和后续API共享稳定
  机器代码；新增或重命名阶段会显式触发契约和测试修改。
- 规则先验证订单/交付父ID、任务集合、每任务进度/质检/复核归属、非空生产步骤和交付记录。
  关键事实缺失统一返回`INSUFFICIENT_INFORMATION`，不能把“没查到”解释成“没有异常”。
- 规则按业务链上最早阶段优先。若生产失败、质检开放和交付阻塞同时存在，返回
  `PRODUCTION_BLOCKED`；否则给出下游阶段会掩盖真正先要处理的阻塞。
- `RuleDecision`与`DiagnosisResult`分离：前者只保存不可被模型改变的机器裁决，后者由M2.7组合
  根因、字段证据、建议和表达。这避免文案生成反向污染业务判断。
- `diagnose_by_rules`不再调用Tool，只消费同一状态快照，并记录`RULE` Step及白名单结果摘要，
  方便按Run确认究竟执行了哪条确定性阶段判断。
- 纯函数规则没有HTTP、数据库和模型依赖，参数化测试可快速覆盖优先级、缺失事实和状态组合；
  Java/API端到端验证仍需在M2.8～M2.10补齐。

### 可能的面试问题及回答要点

**为什么已经要做Agent，还要写确定性规则？**

回答要点：模型适合意图理解和自然语言归纳，不适合拥有生产状态裁决权。先用规则建立可重复的
业务基线，既降低幻觉，又能为后续动态Agent提供黄金评测；模型输出若与规则阶段冲突，应以规则
和Tool事实为准。

**为什么信息不足是一个正式阶段，而不是抛异常或返回NONE？**

回答要点：Tool调用失败仍走`StepError`并中断；而已进入规则时也可能遇到空关键集合或父子归属
矛盾，这属于“无法可靠诊断”。`NONE`表示证据完整且确实无阻塞，两者必须区分，否则缺数据会被
误报成业务正常。

**规则为什么有固定优先级？**

回答要点：订单履约是生产→质检→复核→交付的有序链。多个异常同时存在时，用户首先需要处理
最上游阻塞；固定优先级让输出可解释、可复现，也避免数组顺序或条件代码位置意外改变结论。

**为什么不直接在规则里生成最终DiagnosisResult？**

回答要点：阶段判断和用户表达变化频率不同。`RuleDecision`是稳定机器裁决；根因、Evidence、建议
需要在M2.7绑定具体字段和文案。分层后可以替换规则文案或增加模型润色，而不让模型改变阶段。

### 简历表述建议

> 为订单诊断Agent实现确定性业务裁决层，以强类型Tool事实和父子归属完整性门禁防止缺失数据
> 误判；按生产、质检、复核、交付优先级输出稳定BlockingStage，并将机器RuleDecision与LLM文案
> 生成解耦，通过五场景和异常组合参数化测试建立可重复评测基线。

### 不能过度声称

- 当前五订单是基于与固定数据一致的强类型规则夹具验证，尚未通过诊断HTTP API逐单请求真实Java
  服务；完整E2E属于M2.8～M2.10。
- M2.7尚未实现，当前没有RootCause、Evidence、Suggestion、置信度或最终用户文案。
- 规则没有版本号、灰度发布、配置中心或业务人员可视化编辑能力，不能声称是通用规则引擎。
- 当前`INSUFFICIENT_INFORMATION`只指出信息不足阶段，尚未列出具体缺失字段；需由M2.7文案或
  后续结构扩展表达。
- 没有LLM、RAG、动态Tool选择或Agent自主规划；这里是Agent可靠性基线，不是自主Agent本身。
