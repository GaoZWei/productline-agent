...
## M7：完整 Run/Step、SSE 与回归评测

### 目标

将前面分散的日志、测试和评测统一为完整观测与回归体系。
...
| 迭代       |    时间 | 可演示结果        |
| -------- | ----: | ------------ |
| M7 观测与评测 | 5～7 天 | 可查看链路并运行统一评测 |

整体建议周期约为 6～8 周。

如果时间有限，优先完成 M0～M4，再补 M6。M5 的动态 Agent 可以在固定 Workflow 稳定后逐步增强。
...
在测试报告未达到这些数据之前，简历指标应标记为目标值，不能当作已经完成的真实结果。
