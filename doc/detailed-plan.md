...
统一三个服务的目录、配置、启动和验证方式，形成所有后续开发共同依赖的运行基线；本阶段只建立工程
承载能力，不定义业务事实或Agent决策。
...
在Java业务层建立全系统共享的业务词汇、对象关系和状态约束，使数据库、接口及上层Tool对同一业务事实
使用一致语义，并由业务层而非模型决定状态是否合法。
...
为跨服务开发提供不会随运行顺序或当前时间漂移的事实样本和结果基准，使接口、Tool、Workflow及E2E测试
能够围绕同一业务快照定位回归。
...
把Java业务层确立为订单诊断事实的唯一出口，向Python提供按资源和权限校验后的只读数据；Agent只能归纳
这些响应，不能绕过接口读取业务表或自行补造事实。
...
把最终业务变更收口在Java事务边界内，由Java在写入时重新判断身份、权限、状态、版本和幂等性；当前只
提供受保护的业务能力，不允许Agent绕过人工确认直接执行。
...
建立Java与Python之间稳定的失败语义协议，把HTTP状态和业务异常转换成可路由、可观测的机器契约，供
Client、Tool和Workflow决定是否重试、终止或向用户解释。
...
在不依赖真实随机故障的前提下，为跨服务异常链路提供可控输入，验证超时、错误映射、响应校验和重试边界；
该能力受开发环境门禁保护，不参与正常业务行为。
...
把Java业务事实映射成用户可查看和切换的订单工作台，同时提供后续PageContext和Agent侧边栏所需的当前
业务对象载体；页面只展示与提示，不成为业务事实来源。
...
提供Agent侧统一的HTTP运行时、配置加载、持久化、迁移、日志和质量门禁，使后续Client、Tool与Workflow
运行在同一套可测试、可观测的服务基础上。
...
作为Agent访问业务服务的唯一网络适配层，把连接、身份、追踪和响应契约等横切逻辑集中处理，让Tool只需
表达业务参数和结果，不重复实现HTTP细节。
...
把不同来源的上游失败收敛为Agent内部稳定错误语言，使Tool、重试策略、Workflow和API共享同一分类，
并保留参数、权限、资源、冲突、瞬时故障和响应结构错误之间的处理差异。
...
建立业务能力进入Agent编排层的标准插件边界，用显式Schema、上下文、风险和权限元数据约束调用；Registry
只暴露已注册Tool，避免Workflow或模型任意调用未声明能力。
...
把Java DTO转换成Agent可消费的类型化事实，同时保留资源归属、权限和失败语义；它们是Workflow取证入口，
只读取权威业务状态，不负责判断根因或生成建议。
...
在只读、瞬时且仍有预算的条件下提升取证可靠性，并把重试次数和等待时间限制在明确范围内；确定性错误
和写操作不通过该机制掩盖或重复执行。
...
作为Run内的循环保护门禁，识别同一Tool与参数的重复取证并显式拒绝，防止编排错误持续消耗上游资源；
它不是跨Run缓存，需要最新事实时仍可显式刷新。
...
提供Java接口到Python Client再到Tool的独立验收入口，使Tool能够在编排接入前定位参数、权限、响应和
错误映射问题；接口只在开发环境开放，不构成生产业务API。
...
为Agent编排建立自有的会话和执行账本，保存最小上下文、请求、Run及Step关系；这些表承载运行状态和
可观测信息，不映射或替代Java业务事实表。
...
把一次Agent执行包装成可追踪的顶层状态机，统一开始、成功和失败的终态写入，并通过条件更新阻止并发
路径静默覆盖结果，为API响应和历史追踪提供执行级依据。
...
把Run内部的上下文处理、Tool取证、规则判断和模型整理拆成可审查步骤，使失败能够定位到具体节点；摘要
只保存诊断所需的受控信息，不把密钥或完整敏感响应写入运行记录。
...
作为固定Workflow各节点之间的类型化数据总线，明确区分原始业务事实、诊断结论、字段证据和错误通道；
节点只能通过约定字段交换数据，避免用松散字典或自然语言隐式传递关键状态。
...
按预先确定的顺序编排订单诊断取证，把七个只读Tool结果合并为一致的业务快照，并在失败时停止依赖后续
事实的节点；本阶段由代码固定调用路径，不让模型自主选择Tool。
...
把类型化Tool事实映射成唯一阻塞阶段、根因、证据和建议，按固定优先级处理互斥状态并在事实不足时关闭失败；
它是诊断结论的事实裁决层，不依赖模型临场推理。
...
在确定性规则已锁定订单和阻塞阶段后，本阶段生成可读的阶段说明、根因、字段级证据与建议，并允许
可选模型在不能修改业务事实的边界内整理表达；模型不可用或输出无效时稳定回退规则结果。

### 核心作用

把规则裁决转换为用户可阅读、可追溯的诊断表达，并把模型限制为结构化文案整理器；规则事实始终是基线，
模型输出越界、结构无效或调用失败时不会改变结论，只回退到确定性文案。
...
作为前端进入固定诊断链路的HTTP事务边界，负责校验请求、创建运行记录、调用Workflow并把成功或失败
映射为稳定响应；它不直接查询业务表，也不绕过Workflow生成诊断。
...
把当前订单与用户问题接入可演示的诊断入口，并把确定性 Workflow 返回的阻塞环节、根因、字段级证据、
建议和结构化错误清晰呈现；本阶段只展示诊断和建议，不提供多轮会话、SSE 或写操作确认。

### 核心作用

连接订单页面与诊断API，把当前订单提示和用户问题提交给后端，并将诊断结果、证据及失败位置以可审查
形式展示；侧边栏不裁决业务事实，也不执行建议中的写操作。
...
用隔离 PostgreSQL 和真实 Java 服务验证五个固定订单、运行持久化及关键失败链路，证明诊断阶段、
字段级证据和失败 Step 在跨服务边界上仍符合契约，并为后续上下文与路由开发提供稳定回归基线。

### 核心作用

把固定业务数据、Java接口、Python Tool、Workflow和持久化串成真实跨服务验收链，用黄金场景与故障场景
验证组件组合后的行为，而不只证明单个模块测试通过。
...
把订单、任务和质检页面中的当前业务对象转换为严格上下文提示，并由服务端结合身份与Java Tool事实
重新校验资源归属，为后续会话继承和意图路由提供可信的上下文入口。

### 核心作用

把用户正在查看的页面对象转换为有来源的路由提示，减少重复输入，同时由服务端重新验证身份、资源存在性
和父子归属；页面参数不能直接升级为业务事实或覆盖Java响应。
...
把跨轮次仍有必要的业务指代保存为有界、可过期且按用户隔离的会话上下文，使后续请求能够继承当前
订单或任务，同时继续经过页面Schema和Java Tool事实重校验，不把历史快照冒充最新业务事实。

### 核心作用

在同一用户的连续请求之间保存最小业务指针、已确认参数和待澄清状态，让“这个订单”等指代可以延续；
它只保存路由上下文，不缓存可用于裁决的完整业务事实，并通过所有权和TTL限制复用范围。
...
建立自然语言请求到系统能力之间的有限路由词汇，明确每类意图需要哪些实体、可以映射到哪个Skill以及
何时必须停止澄清，使模型不能通过任意字符串创造新动作。
...
作为自然语言与确定性路由契约之间的模型适配层，向模型提供分源上下文并只接受严格结构化结果；它负责
候选意图和实体提取，不拥有最终执行权，异常时回到不可分发的安全状态。
...
把模型从本轮消息提取的实体与已确认会话、当前页面和临时候选参数分源合并；固定优先级保证显式输入
不会被上下文覆盖，同级多值冲突保持未解析，供后续澄清状态机安全处理。

### 核心作用

把用户消息、已确认会话、页面提示和临时候选中的实体按确定性来源优先级合并，保留每个结果的来源与冲突；
该层决定参数采用哪个可信候选，不允许模型自行覆盖显式输入或猜测同级冲突。
...
把模型意图、服务端实体合并结果和固定置信度阈值组合成不可绕过的最终路由决策；缺参、同级冲突和
不可靠意图生成确定性澄清，用户补参或确认后可恢复原意图而不重复调用模型。

### 核心作用

作为路由结果进入业务Skill前的最终门禁，综合置信度、参数完整性和冲突状态决定执行、确认或澄清；用户
补充的信息以明确来源重新合并，使未满足条件的模型结果不能直接分发。
...
用固定分布的路由样本和可注入执行器重复衡量意图、实体、决策与澄清结果，并以混淆矩阵和脱敏失败样本
定位回归；具体模型未接入时只验收评测基础设施，不把测试替身结果当成真实模型指标。

### 核心作用

把路由质量从主观对话体验转换为可重复的数据集、执行接口和诊断输出，持续检查意图、实体与澄清链路；
评测框架负责暴露回归，不在缺少真实Provider数据时虚构模型准确率。
...
建立可重复校验的演示规范目录、检索元数据和新旧版本关系，为后续文档入库、有效期过滤和带引用检索提供
稳定输入；本阶段不实现数据库、文档解析或RAG召回。

### 核心作用

提供RAG链路可审查、可版本化的知识源基线，把正文文件与产品、卫星、类型、版本、有效期和权限元数据
显式关联；它定义输入语料及关系，不承担解析、索引或检索。
...
把规范文档身份、适用范围、生命周期和Chunk内容映射为Agent自有数据库契约，为全文与向量索引提供统一
落点；知识表保存规范依据，不映射Java订单等业务事实。
...
将受控目录中的Markdown或纯文本文档规范化为带完整章节路径的稳定分块，并在入库前识别重复正文；
后续Embedding与检索可以复用确定性内容哈希和Chunk ID，不必重新解释源文件结构。

### 核心作用

把源文档转换成可索引的确定性Chunk，保留章节路径、规范化正文、内容哈希和稳定身份，并在外部模型或
数据库写入前拦截重复内容；它连接原始语料与后续索引管线。
...
把稳定Chunk批量转换成带Provider、模型、维度和版本身份的向量，并在全部生成成功后短事务写入；它为
语义检索建立可重建、可隔离的向量空间，避免部分批次或跨版本向量污染索引。
...
让中文规范正文和章节标题可以通过确定性双字词元进入PostgreSQL全文检索，并返回可排序的关键词相关度；
后续混合检索可复用稳定Chunk身份和独立关键词分数，不需要让模型解释原始SQL查询。

### 核心作用

提供不依赖模型的字面召回通道，把受控查询词元与GIN全文索引匹配并返回独立关键词分数；结果保留Chunk
身份和原文，作为后续融合检索中可解释的一路候选。
...
使用与文档完全相同的Provider和索引身份生成查询向量，并通过pgvector余弦距离、TopK和阈值召回语义相关
Chunk；查询不会跨模型或索引版本比较，从而避免不同向量空间产生无意义分数。

### 核心作用

提供能发现不同措辞相近语义的召回通道，在相同索引身份内按余弦距离筛选Chunk并对外返回统一相似度；
它补充关键词召回，但不独立决定规范是否适用于当前业务。
...
在关键词和向量候选排序前统一限制产品、卫星、文档类型、规范版本、生效时间和权限范围，防止跨产品、
历史或无权限规范进入后续混合检索与结论生成。

### 核心作用

先判断“这份规范能不能用于当前业务”，再比较“它和用户问题有多相关”；该门禁由关键词和向量Repository
共享，并在排序与TopK前收紧生命周期、时效、权限和可选业务范围，防止某条召回通道绕过适用性约束。
...
关键词相关度和向量相似度量纲不同，且同一Chunk可能被两路重复召回；本阶段用确定性排名融合、去重和
相邻片段合并生成统一候选，避免直接相加不可比分数或让重复结果占据TopK。

### 核心作用

承接已通过元数据门禁的关键词与向量候选，按RRF融合两路名次并保留原始分数、通道排名和全部Chunk身份；
它输出供后续Rerank使用的稳定候选，但不判断最终相关性，也不生成规范结论。
...
RRF只能融合关键词与向量通道名次，无法直接判断候选对当前问题的最终语义相关性；本阶段引入严格可校验的
模型分数，同时隔离超时和异常响应，避免低相关片段进入后续上下文。

### 核心作用

承接M4.8的`RetrievalResult`，在保留原始召回证据的前提下生成重排分数、稳定新顺序和低相关门禁结果；
模型超时会显式降级到原RRF顺序，结构或身份不可信的响应关闭失败，输出供后续引用和上下文组装使用。
