...
检索正文如果缺少规范名称、版本、章节和Chunk身份，用户无法判断回答依据是否适用，也无法回看合并片段的
原文来源；本阶段建立后端严格引用和可展开的前端引用卡片。

### 核心作用

把M4.9重排结果转换为可追溯`Citation`，保留文档身份、章节、全部合并Chunk、正文及真实重排分数；它是
检索结果与带引用生成、前端审查之间的稳定契约，重排未评分时保持空值而不混用RRF分数。
...
已有检索组件仍是分离能力，无法从`SPEC_QA`路由决策稳定执行到带引用回答，也没有在无结果、重排超时或
生成引用异常时阻止模型形成无依据的规范结论。

### 核心作用

用固定LangGraph串联查询改写、页面提示合并、双路召回、RRF、Rerank、充足性检查和带引用生成，并通过
`SpecificationSkill`只接受可分发的`SPEC_QA`决策；任何无法形成可信引用的路径都返回不含规范结论的安全回答。
...
用固定问题、预期文档和预期章节建立可重复的检索质量基线，避免只凭少量演示查询判断RAG效果；同时用统一口径比较四种策略并保留不含问题正文的失败样本。

### 核心作用

位于安全召回链路之后，把关键词、向量、RRF和Reranker的有序结果转换为Hit@5、MRR和无关片段占比；评测只认“文档与完整章节同时命中”，复用生产过滤条件，但不把测试替身指标冒充真实模型质量。
...
固定诊断状态只能保存预定节点读取的业务事实，无法描述动态Agent选择过哪些动作、获得了哪些新信息、仍缺少
什么信息以及为何停止，后续循环决策和安全限额因此缺少稳定状态依据。

### 核心作用

本阶段定义动态诊断循环的共享状态契约，以安全Observation和调用指纹记录Tool历史，并显式保存信息缺口、
迭代次数和终止原因。业务事实仍保留在既有强类型字段中，历史不复制原始Tool载荷，为后续动作模型、动态图
和执行限制提供可序列化的状态边界。
...
动态Agent缺少可校验的下一步动作输出，模型可能选择未知或写入型Tool、混用动作与参数、重复查询已有事实，
也可能在异常响应中把自由文本直接带入执行层。

### 核心作用

本阶段把已验证业务事实、信息缺口、Tool历史和实际可用的只读执行器目录编码为中文结构化Prompt，并将模型
输出收口为动作、执行器与参数严格一致且资源ID来自当前状态的`ActionDecision`。非法输出只纠错一次，仍不
可信或模型异常时返回不执行外部调用的安全`FINISH`，为动态图的决策节点建立关闭失败边界。
...
把单次动作决策连接为可回环、可观察且有明确出口的执行图，避免模型输出绕过校验直接调用Tool，或在执行
失败后继续生成看似完整的业务结论。

### 核心作用

本阶段是动态诊断的执行编排层：上游接收M5.2的结构化动作，下游只通过已注册只读Tool或规范问答Workflow
获得强类型结果，并将Observation、Java事实、终止原因和规则诊断写回状态。模型负责选择下一步，资源校验、
事实合并、完成判断和异常收口仍由确定性节点控制。
...
为可回环动态图增加不可由模型绕过的资源预算和动作门禁，防止无限决策、重复查询、持续无新增信息以及
未知、写入或无权限Tool被执行。

### 核心作用

本阶段是动态循环的确定性保险丝：在决策前、Tool执行前和Observation保存后分别检查轮数、调用预算、
调用指纹、新信息及权限边界，并把预算耗尽与非法执行分流到可审计的安全终止结果。
...
动态Agent不能依赖模型主观判断事实是否足够, 否则可能在缺少关键业务阶段信息时提前结束, 或为了满足
固定全量要求查询与当前场景无关的数据。

### 核心作用

本阶段在Observation与下一轮决策之间计算稳定信息缺口: 先校验订单、任务和交付基础事实, 再按生产、
质检、复核和规范场景追加要求。缺口直接进入动作Prompt和结束判断, 只有当前场景的必需事实齐全时才允许
动态规则生成结论, 固定Workflow的全量事实边界保持不变。
...
单节点和限制测试不能证明动态Agent会针对不同业务阶段选择最小必要路径, 也不能证明动态结果与固定Workflow
基线一致。需要用五个稳定订单覆盖完整循环, 并把重试、去重、预算和写Tool拦截固化为回归门禁。

### 核心作用

本阶段是M5动态诊断链路的场景验收层: 用ORDER-001～005验证场景化缺口如何驱动不同只读Tool组合, 再与
固定Workflow结果逐项对齐。路径中的交付查询属于M5.5基础事实要求；超时只在BaseTool内部有限重试, 图中
仍保存一条逻辑Observation。
...
把Approval中的通用JSON收紧为可验证的复核草稿，防止模型或用户提交非最终结论、超长文案、重复或畸形引用、
相互矛盾的返工建议，以及修改草稿时偷换目标任务。

### 核心作用

ReviewDraft是草稿生成、人工编辑和后续写Tool参数映射之间的统一契约：复用RAG Citation保留规范版本与Chunk
来源，并将Java写接口的文案长度和最终结论边界提前到Approval入口校验。本阶段只校验和持久化草稿，不判断
引用当前有效性，也不执行Java写操作。
