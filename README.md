...
当前进度为 **M1.6 Tool 重试**：Python 服务已通过七个独立只读 Tool 覆盖订单、任务、生产进度、
质检、复核和交付查询，并以严格 Schema 约束 Java 业务事实；只读 Tool 已对明确可恢复的网络
错误和超时执行一次有限退避重试。当前尚未实现重复调用检测、Tool 调试 API、Workflow 或模型调用。
...
make test-tools        # 验证 M1.4 协议、M1.5 七个只读 Tool和M1.6重试策略
