...
test-agent-review: ## 验证 M7.6-F Review草稿、独立Approval、确认/取消和返工闭环
	cd agent-service && uv run --frozen pytest -q tests/test_review_draft_generation.py tests/test_approval_confirmation.py tests/test_approval_confirmation_api.py tests/test_approval_security.py tests/test_production_agent_skills.py
	./scripts/test-agent-persistence.sh -k "review_draft_store or confirmation_store or rework_request"
	cd web-console && npm test -- --run src/api/agentClient.spec.ts src/api/runHistoryClient.spec.ts src/components/ReviewApprovalCard.spec.ts

test-agent-page: ## 验证 M7.6-G统一抽屉、五类结果、四Skill集成和Docker页面链路
	cd web-console && npm test -- --run src/api/agentUnifiedClient.spec.ts src/api/runEventClient.spec.ts src/components/AgentResultView.spec.ts src/components/AgentWorkspaceDrawer.spec.ts src/components/ReviewApprovalCard.spec.ts src/App.spec.ts src/server.spec.mjs
...
test-agent-remaining-fault-matrix: ## 验证 M7.7 场景09～24 Tool/RAG/模型/Agent/Approval/SSE故障
	cd agent-service && uv run --frozen pytest -q tests/test_tool_protocol.py tests/test_tool_call_deduplication.py tests/test_dynamic_diagnosis_workflow.py tests/knowledge/test_specification_qa_workflow.py tests/knowledge/test_reranking.py tests/test_model_client.py tests/test_model_invocation.py tests/test_production_agent_skills.py tests/test_approval_security.py tests/test_run_events.py -k 'm77_s or test_specification_failures_record_stable_rag_step or test_model_protocol_failures_leave_a_stable_failed_llm_step'

test-agent-fault-matrix: test-agent-java-fault-matrix test-agent-remaining-fault-matrix ## 验证 M7.7 全部24个固定异常场景
...
test-eval-framework: ## 验证 M7.8 七类Suite、统一Schema、报告导出与对比
	cd agent-service && uv run --frozen pytest -q tests/evaluation/test_eval_runner.py tests/evaluation/test_acceptance_eval.py tests/evaluation/test_suites.py tests/evaluation/test_reporting.py tests/evaluation/test_evaluation_report_cli.py

eval-report: ## 渲染或比较统一评测报告, 参数通过ARGS传入
	cd agent-service && uv run --frozen python -m app.cli.evaluation_report $(ARGS)

test-eval-metrics: ## 验证 M7.9 全部领域指标口径、计数与边界
	cd agent-service && uv run --frozen pytest -q tests/evaluation

eval-all: test-eval-framework test-eval-metrics ## 运行统一框架与全部领域指标的可重复离线验收
