...
`V1 → V2 → V3` 顺序建表、写入数据并增加写安全结构；M0.4/M0.5 Java 接口和 M1
Python Tool 必须把这里的 ID、状态和值作为契约测试输入，不能由模型补造或随机化。

M0.4 已提供业务 HTTP 查询接口，M0.5 已提供复核和返工写接口。写接口测试会在每个
用例后删除新增记录并把任务版本恢复为 `0`，避免改变固定场景或让测试依赖执行顺序。
...
M0.3 的校验器只返回稳定违规代码，不自动修改数据。M0.5 的两个接口不会迁移订单、
问题或交付状态，因此采用与操作直接相关的任务状态、问题归属、复核结论和重复返工
校验；当后续写操作真正改变跨对象状态时，再把全局一致性校验接入事务门禁并由 M0.6
映射稳定错误码。
