...
由更低优先级提示覆盖。合并结果仍是内部契约；M3.6在下述决策层组合置信度与澄清，HTTP入口、具体模型
供应商和Run/Step持久化仍未实现。

M3.6要求模型返回的每个实体都能在本轮`user_message`中通过大小写不敏感的独立文本匹配；缺少原文证据的
实体按非法模型输出处理。服务端基于合并结果重新计算必填参数，并按以下固定顺序生成`RoutingDecision`：
...
`confidence >= 0.85`为高置信度，`0.60 <= confidence < 0.85`为中置信度，其余为低置信度。候选冲突会
返回带来源的选项；用户只能选择当前待澄清字段和已展示候选，缺参场景可提交符合目标字段Schema的新值。
确认或补参后保留原意图和置信度并重新运行确定性门禁，不重复调用模型。当前没有路由HTTP接口、具体模型
供应商或持久化路由Run；评测基础设施由M3.7实现。

M3.7固定数据集使用JSONL，每行包含用例ID、八类场景之一、用户消息、可选页面/会话上下文以及期望意图、
实体、决策状态和澄清原因。计划分布固定为15条明确意图、10条同义表达、10条页面指代、5条会话指代、
8条缺参、5条多候选、4条意图混淆和3条无关请求。评测Subject只需实现异步`predict(case)`并返回最终
预测；执行器计算意图准确率、参数完整率和完整意图混淆矩阵。失败JSONL不保存`user_message`、
`PageContext`或`SessionContext`，只保留用例ID、类别和结构化期望/实际差异。当前没有真实模型Subject，
测试替身的满分只用于验证执行器，不能作为模型指标。
