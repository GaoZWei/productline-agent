...
`/api` 业务端点在领域 DTO 外统一增加 `ApiResponse<T>` 信封，参数、身份、权限、资源、
业务冲突和系统异常分别稳定映射为 400、401、403、404、409 和 500。错误码与 HTTP
状态共同表达语义，后续 Python Tool 不需要解析错误文案来决定分支。

Trace ID 在请求入口校验或生成，并同时写入请求属性、MDC、响应 Header 和响应体。通用
500 隐藏内部异常详情且标记为不可自动重试；写请求结果可能未知时，调用方必须依靠原
幂等键查询或重放，不能因为 500 就生成新键重复写入。
